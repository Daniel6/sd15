...
beats_per_minute = 45*3.5

curr_note = 0
add_note(solo, bass, blues_scale[curr_note], 1.0, beats_per_minute, 1.0)

licks = [ [ [-3, 0.5*1.1], [-1, 0.5*.8], [-2, 0.5*.6], [-1, 0.5] ],
          [ [3, 0.5*1.1], [-3, 0.5*.8], [3, 0.5*.6], [-3, 0.5] ], 
          [ [1, 0.5*1.1], [1, 0.5*.8], [1, 0.5*.6], [1, 0.5] ], 
          [ [-3, 0.5*2.1], [2, 0.5*.8], [-2, 0.5*.8], [-1, 0.5*.8] ] ]
for i in range(40):
    lick = choice(licks)
    for note in lick:
        curr_note += note[0]
